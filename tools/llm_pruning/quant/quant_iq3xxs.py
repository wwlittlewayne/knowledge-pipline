#!/usr/bin/env python3
"""
Quantize a model to GGUF IQ3_XXS (llama.cpp) — quantization only, no pruning.

IQ3_XXS ≈ 3.06 bits/weight. For GLM-5.2 (753B) ≈ 309 GB — fits the 2x 3090 +
512 GB DDR4 box (560 GB usable). Best quality/size tradeoff among the sub-4-bit
options; recommended default for engineering+coding use where quality matters.
imatrix is on by default (strongly recommended at 3-bit).

Usage:
    python quant_iq3xxs.py --model-id zai-org/GLM-5.2 \\
        --llama-cpp-dir /opt/llama.cpp --out ./out/glm52-iq3xxs
"""
import argparse
from pathlib import Path

from quant_common import run_gguf_quant

FORMAT = "IQ3_XXS"


def main() -> None:
    p = argparse.ArgumentParser(description=f"GGUF {FORMAT} quantization (no pruning)")
    p.add_argument("--model-id", default="zai-org/GLM-5.2", help="HF id or local path")
    p.add_argument("--out", type=Path, default=Path("./out/iq3xxs"))
    p.add_argument("--llama-cpp-dir", type=Path, default=Path("/opt/llama.cpp"))
    p.add_argument("--moe", dest="moe", action="store_true", default=True)
    p.add_argument("--dense", dest="moe", action="store_false")
    p.add_argument("--imatrix-samples", type=int, default=512)
    p.add_argument("--no-imatrix", dest="imatrix", action="store_false", default=True,
                   help="Skip imatrix (NOT recommended at 3-bit)")
    p.add_argument("--keep-f16", action="store_true")
    args = p.parse_args()

    run_gguf_quant(
        model_id=args.model_id,
        out_dir=args.out,
        llama_cpp_dir=args.llama_cpp_dir,
        fmt=FORMAT,
        use_imatrix=args.imatrix,
        is_moe=args.moe,
        imatrix_samples=args.imatrix_samples,
        keep_f16=args.keep_f16,
    )


if __name__ == "__main__":
    main()
